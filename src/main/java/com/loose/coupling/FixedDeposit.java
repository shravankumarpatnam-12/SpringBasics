...
//            LocalDate today = LocalDate.now();
//            Period period = Period.between(openedDate, today);
            double interestAmount=(principalAmount*interestRate*monthsBetween)/(100*12);
            principalAmount=principalAmount+interestAmount;
            System.out.println("Calculated interest amount "+interestAmount);
            System.out.println("balance "+principalAmount);
        }else{
            System.out.println("Fixed deposit account is " +fdNumber+ "closed hence interest cannot be calculated");
        }
    }
